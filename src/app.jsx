...
import { getCreatureAsset, getCreatureAssetStyle, MASTER_CREATURE_ASSET } from './data/assetUrls.mjs'
import {
...
function CreatureArt({ id, className = '' }) {
  const style = getCreatureAssetStyle(id)
  if (!style) return <div className={`silhouette-shape ${className}`}>{(CREATURE_BY_ID[id]?.name || '?')[0]}</div>
  return <div className={`creature-art ${className}`} role="img" aria-label={CREATURE_BY_ID[id]?.name || id} style={style} />
}
...
class MeltVisual extends React.Component {
  ...
  componentDidUpdate(prev) {
    if (prev.creatureId !== this.props.creatureId) { this.controller?.destroy(); this.mountScene(); return }
    if (prev.progress !== this.props.progress) this.controller?.setProgress(this.props.progress)
  }
  mountScene() {
  ...
  render() {
    return <div className="visual-stack">
      ...
      {this.state.fallback && <CreatureArt id={this.props.creatureId} className="fallback-creature-art" />}
    </div>
  }
}
...
class MapVisual extends React.Component {
  ...
  render() {
    return <div className="visual-stack map-fallback-bg">
      ...
      {this.state.fallback && <div className="offline-map-fallback"><img src={MASTER_CREATURE_ASSET} alt="Brachiosaurus" /><span>3D Meadow loads when Three.js is available.</span></div>}
    </div>
  }
}
...
class MelTimeScreen extends React.Component {
  ...
  render() {
    ...
    return <section className="screen meltime-screen">
      <BrandHeader title="MelTime" subtitle="집중한 시간이 얼음을 녹이고, 생명체를 깨웁니다." right={<span className="day-chip">DAY 01</span>} />
      ...
      <div className="revival-summary">
        ...
        <small>{Math.ceil(Math.max(0, creature.requiredMinutes - focused))}분 더 집중하면 완전히 깨어납니다.</small>
      </div>
...
function CollectionScreen({ game, update, goMelTime }) {
  ...
  return <section className="screen collection-screen">
    <BrandHeader title="Collection" subtitle="깨어난 생명체들과 아직 잠든 친구들을 모아보세요." right={<strong className="count-pill">{game.revivedIds.length} / {CREATURES.length}</strong>} />
    <div className="filter-row"><button className="active">All</button><button>Plains</button><button>Snow</button><button>Sky</button><button>Ocean</button></div>
    <div className="collection-grid">{CREATURES.map(creature => {
      ...
      const asset = getCreatureAsset(creature.id)
      return <button key={creature.id} disabled={!asset} className={`collection-card ${game.activeCreatureId===creature.id?'selected-creature':''}`} onClick={() => asset && choose(creature.id)}>
        <div className={`asset-box ${!revived && p === 0 ? 'locked' : ''}`}>{asset ? <CreatureArt id={creature.id} /> : <div className="silhouette-shape">{creature.name[0]}</div>}{revived && <span className="revived-mark">✓</span>}{!revived && p===0 && <span className="lock-mark">◇</span>}</div>
        <div className="collection-copy"><strong>{asset ? creature.name : '???'}</strong><span>{!asset ? 'Coming soon' : revived ? 'Revived' : p>0 ? `${Math.round(p*100)}% awake` : creature.rarity}</span></div>
...
class MapScreen extends React.Component {
  ...
  render() {
    ...
    return <section className="screen map-screen">
      <BrandHeader title="World Map" subtitle="집중으로 깨어난 친구들이 살아가는 3D 세계입니다." right={<span className="count-pill">{game.revivedIds.length} / 6</span>} />
      ...
      <div className="map-info-card"><div><span>MAP 01</span><strong>Sunny Plains</strong></div><p>깨어난 생명체는 이 초원에 자동으로 배치됩니다. 드래그해 둘러보고 공룡을 눌러 다음 집중 대상을 선택하세요.</p></div>
    </section>
  }
}
...
function ProfileScreen({ game }) {
  ...
  return <section className="screen profile-screen">
    ...
    <div className="profile-hero"><div className="avatar-ring"><img src={MASTER_CREATURE_ASSET} alt="Brachiosaurus companion"/></div><div><span>DINOVA EXPLORER</span><strong>Focus Keeper</strong><p>Focus today. Revive the past.</p></div></div>
  ...
  render() {
    ...
    return <main className="app-shell">
      ...
      <div className="app-frame">
        <div className="status-bar"><span>DINOVA</span><span className="status-icons">● ◔ ▰</span></div>
      ...
      <aside className="desktop-story">
        ...
        <div className="story-creature"><img src={MASTER_CREATURE_ASSET} alt="DINOVA master Brachiosaurus"/></div>
